...
cardiff_pipe = pipeline("sentiment-analysis", model="cardiffnlp/twitter-roberta-base-sentiment")
...
def score_finbert(text):
    ...
    return (probs[2] - probs[0]).item()  # pos - neg
...
def analyze_news_sentiment(ticker, lookback_days=1):
    ...
    for article in news_data:
        ...
        cardiff_score = normalize_label_score(cardiff_pipe(headline)[0]["label"])

        scores = [vader_score, finbert_score, sst_score, cardiff_score]
        final_score, confidence = aggregate_scores(scores)

        print(f"[{ticker}] Headline: {headline}\n  VADER: {vader_score:.3f}, FinBERT: {finbert_score:.3f}, SST-2: {sst_score:.3f}, Cardiff: {cardiff_score:.3f}\n  Final Score: {final_score:.3f}, Confidence: {confidence:.2f}\n")
    ...
    #print(f"\n>>> Final Signal for {ticker}: {signal} | Score: {final_avg_score:.3f} | Confidence: {final_confidence:.2f}")
    return {
        "ticker": ticker,
        "signal": signal,
